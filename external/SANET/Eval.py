import argparse
import os
from pathlib import Path  # [local batch]
import torch
...
# [local batch] optional directory mode: full content x style cartesian product
...
args = parser.parse_args()  # [local compat] was parse_args(''), which ignored CLI args
...
# [local batch] pair list: directory mode (cartesian) or legacy single pair
if args.content_dir and args.style_dir:
    pairs = [(c, s) for c in sorted(Path(args.content_dir).glob('*'))
             for s in sorted(Path(args.style_dir).glob('*'))]
else:
    pairs = [(args.content, args.style)]

for content_path, style_path in pairs:  # [local batch] loop over pairs
    ...
    with torch.no_grad():
        ...
        output_name = '{:s}/{:s}_stylized_{:s}{:s}'.format(
                    args.output, splitext(basename(str(content_path)))[0],
                    splitext(basename(str(style_path)))[0], args.save_ext
                )
